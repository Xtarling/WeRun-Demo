...
public class GamePanel extends JPanel implements KeyListener {
    ...
    boolean flag = true; //是否暂停标志。图片最好换一下

    public GamePanel() {
        person = new Person();
        pet = new Pet();
        try {
            background = ImageIO.read(new File("WeRun_v1/image/cc.png"));
            score = ImageIO.read(new File("WeRun_v1/image/a12.png"));
            pause = ImageIO.read(new File("WeRun_v1/image/b2.png"));
            proceed = ImageIO.read(new File("WeRun_v1/image/b1.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void paint(Graphics g) {
        ...
        //绘制背景（交替滚动形式）
        if (flag) backgroundX -= 20; //设置speed变量？
...
//            barrs_2.setY(petY - 30);
        ...
        if (e.getKeyCode() == KeyEvent.VK_SPACE) flag = !flag;
    }
