...
public class EndFrame extends JFrame implements MouseListener {
    ...
    public EndFrame(Person person) {
        again = new JLabel(new ImageIcon("WeRun_v1/image/hh5.png"));
        again.setBounds(520, 622, 60, 25);
        again.addMouseListener(this);
        this.add(again);

        back = new JLabel(new ImageIcon("WeRun_v1/image/hh6.png"));
    ...
    class EndPanel extends JPanel {
        ...
        public EndPanel(Person person) {
            p = person;
            try {
                background = ImageIO.read(new File("WeRun_v1/image/chou.png"));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
