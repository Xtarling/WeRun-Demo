 ...
 * ### v2改动：
 *  1.切换WindowFrame改用新开线程
 *  2.点击exit按钮的响应事件加了System.exit(0);
 */
public class MainFrame extends JFrame implements MouseListener {
    JLabel start, help, exit;

    public MainFrame() {
        start = new JLabel(new ImageIcon("WeRun_v2/image/hh1.png"));
        start.setBounds(350, 320, 150, 40);
        start.setEnabled(false); //按钮为灰色
        start.addMouseListener(this);
        this.add(start);

        help = new JLabel(new ImageIcon("WeRun_v2/image/hh2.png"));
    ...
    class MainPanel extends JPanel {
        ...
        public MainPanel() {
            try {
                background = ImageIO.read(new File("WeRun_v2/image/main.png"));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
